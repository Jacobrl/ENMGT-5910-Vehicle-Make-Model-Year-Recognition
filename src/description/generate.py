...
model = AutoModelForVision2Seq.from_pretrained(
    MODEL_ID,
    dtype=torch.bfloat16,      # safer for CPU
...
def build_prompt(metadata):
    ...
    # Build damage summary text from detections
    damage_lines = []
    for d in damage:
    ...
    # Prepare cost text hints for the model
    usd = cost.get("usd")
    mxn = cost.get("mxn")
...
def generate_description_qwen(metadata, image_paths):
    ...
    messages = [
        {
            "role": "user",
            "content": [
                {"type": "text", "text": build_prompt(metadata)}
            ]
        }
    ]
    ...
    for img_path in image_paths:
        ...
        # Wrap image in conversation-style format
        conv = [{
            "role": "user",
            "content": [
        ...
        # Add them to main message content
